...
type history = {
  amount: number;
  amountType: "Withdraw" | "Deposit";
  id: number;
};
let history_data : history[] = []

const authFlow: { name: string }[] = [
  {
    name: "Signup",
  },
  {
...
const changeName = async () => {
  ...
  name = _name;
  printColoredMessage(
    `New Name has updated successfully your new username is ${colors.yellow}${_name}${colors.reset}`
  );
  getProfileData();
};
...
const changePin = async () => {
  ...
  pin = _pin;
  printColoredMessage(
    `New Pin has updated successfully your new Pin is ${colors.yellow}${_pin}${colors.reset}`
  );
  getProfileData();
};
...
const withdrawamount = async () => {
  let _amount:number = await performTransaction(
...
const printHistoryData = (data: history[]) => {
  if (data.length === 0) {
...
const getProfileData = async () => {
  ...
  printColoredMessage(
    `Welcome back ${name} here you can change passcode,name perform transactions and see history`,
    "green"
  );
  let option: {
    choice:
...
const askName = async (message?: string): Promise<string> => {
  let _name: { name: string } = await inquirer.prompt([
    {
...
const askLoginPin = async (): Promise<boolean> => {
...
const resetPin = async (): Promise<number> => {
  let _pin: { pin: number } = await inquirer.prompt([
    {
...
const askBalance = async ():Promise<number> => {
    ...
    if (askUserName == name) {
      do {
        ...
        if (_loginPin) {
          ...
          attempts += 1;
          printColoredMessage(
            `You entered wrong pin. remaining ${3 - attempts} out of 3`,
            "yellow"
          );
          continue;
        }
      ...
      if (loginPin !== pin) {
        let isResetPin: { resetPin: boolean } = await inquirer.prompt([
          {
            message:
              "you have exceed the attempts limit do you want to Reset the PIN?",
            type: "confirm",
            name: "resetPin",
          },
        ]);
...
const printAfterLoginMessage = () => {
  ...
  const formattedTime: string = getFormattedTodayTime(time.getHours(), time.getMinutes()); // 8 PM
...
const Atm = async (singedUp?: boolean) => {
  if(!singedUp){
  printColoredMessage(
    `********** Welcome to Furqan's Banking App **********`,
    "green"
  );
  const appFeatures = `
  Features:
...
`;
  ...
  let auth: { authFlow: "Signup" | "Login" } = await inquirer.prompt([
    {
      name: "authFlow",
      type: "list",
      choices: authFlow,
...
const getFormattedTodayTime = (hours: number, minutes: number): string => {
  const now = new Date();
